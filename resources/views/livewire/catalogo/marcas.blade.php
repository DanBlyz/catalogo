<div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                    <i class="fa-solid fa-copyright text-brand-500"></i>
                    <span>Gestión de Marcas</span>
                </h1>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    Registro y clasificación de marcas de fabricantes
                </p>
            </div>
            ...
            <nav class="flex text-xs text-slate-400 font-medium">
                ...
                <span class="text-slate-700 dark:text-slate-200">Marcas</span>
            </nav>
        </div>
    </x-slot>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
            ...
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-900/50">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-slate-900 dark:text-white">Listado de Marcas</h2>
                        <p class="text-[11px] text-slate-500 dark:text-slate-400">Total de registros: {{ $marcas->total() }}</p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-100/70 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 text-[11px] font-bold uppercase tracking-wider">
                                ...
                                <th class="py-3 px-4">Nombre de la Marca</th>
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                                    ...
                                    <td class="py-3 px-4 text-center">
                                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20">
                                            <i class="fa-solid fa-box text-[9px]"></i> {{ $marca->productos_count }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex items-center justify-center gap-1.5">
                                            ...
                                            <button 
                                                type="button"
                                                onclick="confirmDeleteMarca({{ $marca->id }}, '{{ addslashes($marca->nombre) }}')" 
                                ...
                                <tr>
                                    <td colspan="6" class="py-8 text-center text-slate-400">
                                        <div class="flex flex-col items-center justify-center gap-2">
                                            <i class="fa-solid fa-copyright text-3xl text-slate-300 dark:text-slate-700"></i>
                                            <p class="text-xs font-medium">No se encontraron marcas registradas</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                    ...
                    <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-xl bg-brand-500/10 text-brand-500 flex items-center justify-center font-bold text-sm">
                                <i class="fa-solid {{ $marcaId ? 'fa-pen-to-square' : 'fa-plus' }}"></i>
                            </div>
                    ...
                    <form wire:submit.prevent="save" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input 
                                type="text" 
                                wire:model="nombre" 
                                placeholder="Ej: Sony, Samsung, Nike, Nestle" 
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                rows="3" 
                                placeholder="Breve descripción de la marca o fabricante..." 
                                class="w-full px-3.5 py-2.5 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-brand-500 focus:outline-none transition"
                            ></textarea>
    ...
    <script>
        ...
        function confirmDeleteMarca(id, name) {
            Swal.fire({
                title: '¿Eliminar Marca?',
                text: `¿Estás seguro de eliminar la marca "${name}"?`,
                icon: 'warning',
